...
resName.addEventListener("click", ()=>{
  ...
  resName.classList.add("res-name-hidden"); // span숨기기 클래스 추가

  resNameInput.classList.remove("res-name-hidden"); //input 숨기기 클래스 제거
...
resNameInput.addEventListener("blur", ()=>{
  ...
  resNameInput.classList.add("res-name-hidden"); // span숨기기 클래스 추가

  resName.classList.remove("res-name-hidden"); //input 숨기기 클래스 제거
...
updateBtn.addEventListener("click", ()=>{
  ...
  // 2) .menu 모두 얻어오기
  const menus = document.querySelectorAll(".menu");   //querySelectorAll은 nodelist를 반환
  ...
  // 3) 모든 .menu를 순차 접근
   menus.forEach(item=>{
    ...
    // 5) .menu 내부에 추가될 input요소를 모아둔 JS 객체 얻어오기
    const menuContent = createMenuContent();

    // 삼항연산자
    // -> 조건식 ? 조건식 true인 경우 : false인 경우;
...
function newEl(tag, attr, cls){
  ...
  // cls 매개 변수는 배열 형태
  for(let className of cls){ //클래스 추가
