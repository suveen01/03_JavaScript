...
btn1.addEventListener("click", ()=>{
  ...
  if( !input1.hasAttribute("placeholder")){
    input1.setAttribute("paceholder", "placehodler 추가됨")
  }
});
...
btn2.addEventListener("click", ()=>{
  ...
  //[tip]
  //브라우저 콘솔에서 특정 요소가 가지고있는 DOM속성을 직접 확인하고 싶은 경우
  // -> 복수 형태로 요소를 얻어오는 queryselectorAll(), geElementByClassName()등을 이용
  ...
  console.log("#input2의 name :", divs[0].name);

  //value 얻어오기
  console.log("#input2의 name :", inputs[0].getAttribute("value"));
  console.log("#input2의 name :", inputs[0].value);
  console.log("#input2의 name :", divs[0].value);
...
// 클래스를 이용한 CSS 스타일 제어하는 방법이 제일 많이 사용됨
  -> HTML 복잡도가 낮아짐(깔끔~)
// 요소.style.css 속성 방법은 inline-style로 적용되기 때문에 HTML의 복잡도가 늘어나는 문제가 발생
    단, class 를 이용한 css를 덮어 씌울 목적인 경우 사용이 가능하다
...
btns.forEach((btn)=>{
  ...
  btn.addEventListener("click", ()=>{
    ...
    // container3 가 가지고 있는 bg-로 시작하는 class 모두 제거
    // == 
...
modifyBtn.addEventListener("click", ()=>{
  ...
  // 클래스 bg-green <-> bg-red 변경하기(replace이용)
