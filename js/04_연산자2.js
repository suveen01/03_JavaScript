...
function multipleCheck(){
  ...
  /* result2라는 요소의 내용으로 결과를 대입해서 출력 */
...
function orCheck(){
  ...
  console.log(`나이가 18세 미만이거나, 이름이 "홍길동" 인가? ${bool2}`);

  //국어 또는 영어점수가 40점 미만이거나 평균이 60점 미만인가?
  const kor = 50;
  const eng = 75;
  const bool3 = ((kor < 40) || (eng < 40)) || (kor + eng / 2 < 60);
  console.log(`국어 또는 영어점수가 40점 미만이거나 평균이 60점 미만인가? ${bool3}`);
}