...
function Student(grade, ban, number, name){
  ...
  //생성된 Student 객체의 key가 grade인 필드에 value로 전달 받은 grade 를 대입
  this.grade = grade;
  this.ban = ban;
  this.number = number;
  this.name = name;
...
/* 학생추가버튼 클릭 시 입력된 값을 모두 얻어와 학생 객체 배열(student list)에 추가하기 */
...
addStudent.addEventListener("click", ()=>{
 ...
 /*  const grade = inputGrade.value;
  ...
  for(let val of values){
    if(val.trim() === ''){ //입력 안한 경우 //trim : 양쪽 공백 문자 제거
      alert(`모든 값을 입력해주세요`);
      return;
    }
  }
...
selectStudent.addEventListener("click", ()=>{
  ...
  tbody.innerHTML = ""; //tbody안에 빈칸을 대입하라(내용초기화)
  ...
  studentList.forEach((std, index)=>{
    // std : studentList 에서 하나씩 꺼낸 학생 객체
    // index : 현재 index
...
/* 
  배열.forEach((item, index) =>{
    // item : for 문 반복 시 0 인덱스 부터 차례대로 꺼낸 요소
